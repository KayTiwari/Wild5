...
class HeroOpt extends Component{
    ...
    feeling = () => {
        if (this.state.optval === 0){
            return <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%', color:'red'}}>Not at all Optimistic</Text>
        } else if (this.state.optval >= 1 && this.state.optval <= 3){
            return <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%', color:'orange'}}>Mildly Optimistic</Text>
        } else if (this.state.optval >= 4 && this.state.optval <= 6){
            return <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%', color:'#fad859'}}>Moderately Optimistic</Text>
        } else if (this.state.optval >= 7 && this.state.optval <= 9){
            return <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%', color:'#2ecc71'}}>Highly Optimistic</Text>
        } else if (this.state.optval === 10){
            return <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%', color: '#52b3d9'}}>Extremely Optimistic</Text>
        }
    }
        ...
        return (
            <View style={{backgroundColor: 'white', height: screenheight}}>
                ...
                <View style={{ flex: 1, alignItems: 'stretch', marginLeft:'5%', marginRight:'5%', marginTop:'10%'}}>
                    ...
                    />
                    <Text style={{fontSize: 25, fontWeight: '600', textAlign: 'center', marginTop: '10%'}}>Value: {this.state.optval}</Text>
                    {this.feeling()}
